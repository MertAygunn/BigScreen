...
#define LOGO_GENISLIK 120
#define LOGO_YUKSEKLIK 180
...
#define OK_GENISLIK 112
#define OK_YUKSEKLIK 97
...
void logoCizdir(const unsigned char* logo, bool isWeather, String changePercent24Hr) {
  if (isWeather) {
    if (changePercent24Hr == "Clouds") {
      tft.drawBitmap(68, 27, clouds, 60, 90, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Snow") {
      tft.drawBitmap(68, 27, snow, 60, 90, ILI9341_WHITE);
     
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Thunderstorm") {
      tft.drawBitmap(68, 27, thunderstorm, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Drizzle") {
      tft.drawBitmap(68, 27, drizzle, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Rain") {
      tft.drawBitmap(68, 27, rain, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Clear") {
      tft.drawBitmap(68, 27, sunny, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Mist") {
      tft.drawBitmap(68, 27, mist, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Smoke") {
      tft.drawBitmap(68, 27, smoke, 24, 24, ILI9341_WHITE);
     
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Haze") {
      tft.drawBitmap(68, 27, haze, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Dust") {
      tft.drawBitmap(68, 27, dust, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Fog") {
      tft.drawBitmap(68, 27, fog, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Sand") {
      tft.drawBitmap(68, 27, sand, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Ash") {
      tft.drawBitmap(68, 27, ash, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Squall") {
      tft.drawBitmap(68, 27, squall, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } else if (changePercent24Hr == "Tornado") {
      tft.drawBitmap(68, 27, tornado, 24, 24, ILI9341_WHITE);
      
      Serial.print("Hava durumu logo cizildi");
    } 
    else {
      tft.drawBitmap(0, (tft.height() + (tft.height() / 4) - LOGO_YUKSEKLIK) / 2, logo, LOGO_GENISLIK, LOGO_YUKSEKLIK, ILI9341_WHITE);
...
void okCizdir(String changePercent24Hr, bool isWeather) {
  if (isWeather) {
    tft.drawLine(0, 45, 320, 45, ILI9341_YELLOW);
    tft.drawRoundRect(2, 50, 235, 270, 5, ILI9341_MAGENTA);
    ...
    if (isNumeric) {
      if (changePercent24Hr.toFloat() >= 0) {
        tft.drawBitmap(tft.width() - OK_GENISLIK, tft.height() - OK_YUKSEKLIK, stonks, OK_GENISLIK, OK_YUKSEKLIK, ILI9341_WHITE);
        Serial.println("Stonks oku çizildi");
      } else {
        tft.drawBitmap(tft.width() - OK_GENISLIK, tft.height() - OK_YUKSEKLIK, notstonks, OK_GENISLIK, OK_YUKSEKLIK, ILI9341_WHITE);
...
void degerYazdir(String value, bool isWeather) {
  ...
  if (isWeather) {
    if (value.length() <= 5) {
      ...
      tft.setTextSize(2);
      tft.print(valueBuf);
      tft.drawBitmap(10, 22, termometre, 18, 30, ILI9341_WHITE);
    } else {
    ...
    valueBuf = value.substring(0, 8) + "$";
      int16_t x1, y1;
      uint16_t w, h;
      tft.getTextBounds(valueBuf, 0, 0, &x1, &y1, &w, &h);
...
void degisimiYazdir(String changePercent24Hr, bool isWeather) {
  if (!isWeather) {
    ...
    tft.setCursor(LOGO_GENISLIK + 10, tft.height() / 4 + 2);
    tft.print(changeStr);
...
void isminiYazdir(String symbol, bool isWeather, String saat, String nem) {
  if (isWeather) {
    ...
    tft.setCursor(LOGO_GENISLIK + 5, tft.height() / 4 * 3);
    tft.print(symbol);
...
void setup() {
  Serial.begin(115200);
  tft.begin();
  tft.setRotation(0); // Adjust the rotation to match your screen orientation
